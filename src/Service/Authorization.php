...
class Authorization {
    ...
    public static function login(string $login, string $password) {
        $user = Registration::entityManager()
    ...
    public static function register(string $login, string $password) {
        $user_exists = Registration::entityManager()->getRepository(User::class)->count(["login" => $login]) != 0;
        if ($user_exists) {
        ...
        try {
            Registration::entityManager()->persist($user);
    ...
    protected static function loadUserFromSession() {
        if (!isset(self::$auth_user) && isset($_SESSION['USER'])) {
            self::$auth_user = Registration::entityManager()
                ->getRepository(User::class)
